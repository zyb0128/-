...
class buy_1Logic {
	 ...
	 * 水币支付,依次循环每个订单
	 */
	public function sbPay($order_list, $input, $buyer_info){
		$member_id = $buyer_info['member_id'];
		$member_name = $buyer_info['member_name'];
		

		$available_pd_amount = floatval(ncPriceFormat($buyer_info['water_fee']));
		if ($available_pd_amount <= 0) return $order_list;
		...
		foreach ($order_list as $key => $order_info) {
			...
			$order_amount = floatval($order_info['order_amount']) - floatval($order_info['sb_amount']);
			$order_amount = floatval(ncPriceFormat($order_amount));
			$data_pd = array();
			$data_pd['member_id'] = $member_id;
			$data_pd['member_name'] = $member_name;
			...
			if ($available_pd_amount >= $order_amount) {
				//预存款立即支付，订单支付完成
				$model_pd->changePd('order_sb_pay',$data_pd);
				$available_pd_amount -= $order_amount;
				$available_pd_amount = floatval(ncPriceFormat($available_pd_amount));
				...
				$order_list[$key]['order_state'] = $data_order['order_state'] = ORDER_STATE_PAY;
				$order_list[$key]['payment_time'] = $data_order['payment_time'] = TIMESTAMP;
				$order_list[$key]['payment_code'] = $data_order['payment_code'] = 'sbpay';
				if ($order_info['order_type'] == 2) {
					$order_list[$key]['sb_amount'] = $data_order['sb_amount'] = ncPriceFormat(floatval($order_info['original_sb_amount']) + $order_amount);
				} else {
					$order_list[$key]['sb_amount'] = $data_order['sb_amount'] = $order_amount;
				}
