...
class rechargecardModel extends Model
{
	...
	public function addRechargewater($param){
		return $this->table('rechargewater')->insert($param);
	}


	public function getRechargeWaterList($condition, $pageSize = 20, $limit = '', $sort = 'activityId desc'){
		return $this->table('rechargewater')->where($condition)->limit($limit)->page($pageSize)->order($sort)->select();
	}
