...
class refund_returnModel extends Model{
	...
	public function editOrderRefund($refund,$user = '') {
		...
		if ($refund_id > 0) {
			...
			$field = 'order_id,buyer_id,buyer_name,store_id,store_name,order_sn,order_amount,payment_code,order_state,refund_amount,rcb_amount,pd_amount,rpt_amount,shipping_fee,sb_amount';
			$model_order = Model('order');
			...
			try {
				...
				$sb_amount =  $order['sb_amount'];//水币支付金额
				$predeposit_amount = $order_amount-$order['refund_amount']-$rcb_amount;//可退预存款金额
				$detail_array = array();

				if ( ($sb_amount > 0) && ($order_amount == $sb_amount) ) {//水币退款
					$log_array = array();
					...
					$detail_array['rcb_amount'] = $log_array['amount'];//金额
					// $state = $model_predeposit->changeRcb('refund', $log_array);//增加买家可用水币金额

					//减少商家水币金额
					$data_sb['water_fee'] = array('exp','water_fee-'.$log_array['amount']);
					Model('store') -> editStore($data_sb,array('store_id'=>$order['store_id']));

					//商家水币记录（减少）
					$log_sb_add = array();
					$log_sb_add['log_store_name'] = $order['store_name'];
					$log_sb_add['log_store_id'] = $order['store_id'];
					$log_sb_add['log_type'] = 'sb_back';
					$log_sb_add['log_fee'] = '-'.$log_array['amount'];
					$log_sb_add['log_add_time'] = time();
					$log_sb_add['log_desc'] = "确认退水币，订单号: ".$order['order_sn'];
					Model('store') -> addStoreSbLog($log_sb_add);

					if ($log_array['amount'] > 0) {
						$detail_array['pd_amount'] = $log_array['amount'];
						$state = $model_predeposit->changePd('refund_sb', $log_array);//增加买家可用水币金额
					}
				}else{
