	...
	<form action="index.php?act=payment&op=real_order" method="POST" id="buy_form">
	 ...
	 还需在线支付 <strong id="api_pay_amount"><?php echo ncPriceFormat($output['pay']['payd_diff_amount']);?></strong>元。</dd>
			<?php } ?>
				...
				<dt>使用水币支付</dt>
				<dd>
					<label><input type="checkbox" class="checkbox" value="1" name="sb_pay">
					使用水币支付</label> （可用余额：<em><?php echo ncPriceFormat($output['user']['water_fee']);?></em>元）</dd>
				<dd>
...
var member_sb = <?php echo $output['user']['water_fee'];?>;
...
$(function(){
		...
		$('#next_button').on('click',function(){
			if (($('input[name="pd_pay"]').attr('checked') || $('input[name="rcb_pay"]').attr('checked') || $('input[name="sb_pay"]').attr('checked')) && $('#password_callback').val() != '1') {
		...
		<?php if ($output['pay']['if_show_pdrcb_select']) { ?>
				function showPaySubmit() {
						if ($('input[name="pd_pay"]').attr('checked') || $('input[name="rcb_pay"]').attr('checked') || $('input[name="sb_pay"]').attr('checked')) {
							$('#pay-password').val('');
					...
					if ($('input[name="sb_pay"]').attr('checked')) {
						_diff_amount -= member_sb;
					}
					_diff_amount = parseFloat(_diff_amount.toFixed(2));
				...
				$('input[name="sb_pay"]').on('change',function(){
					showPaySubmit();
				});
		<?php } ?>
});
