...
Tpl::setDirquna('shop');
	 ...
	 * 增加店铺水币值
	 */
	public function add_sbOp(){
		$store_id = intval($_GET['id']); //店铺ID
		$storeModel = model('store');
		$storeArray = $storeModel->where(array('store_id'=>$store_id))->find();

		//增加水币
		if (chksubmit()) {
			$store_id = $_POST['store_id'];
			$sb_num = round($_POST['sb_num']); //数值

			$storeinfo = Model('store') -> getStoreInfoByID($store_id);

			$data_sb['water_fee'] = array('exp','water_fee+'.$sb_num);
			$staue = Model('store') -> editStore($data_sb,array('store_id'=>$store_id));

			//水币充值记录
			$log_sb_add = array();
			$log_sb_add['log_store_name'] = $storeinfo['store_name'];
			$log_sb_add['log_store_id'] = $store_id;
			$log_sb_add['log_type'] = 'sb_top';
			$log_sb_add['log_fee'] = $sb_num;
			$log_sb_add['log_add_time'] = time();
			$log_sb_add['log_desc'] = "确认获得水币，金额：". $sb_num;
			Model('store') -> addStoreSbLog($log_sb_add);

			$this->log("为自营店铺增加水币值: {$log_sb_add['store_name']}");
			showMessage('操作成功', urlAdminShop('ownshop', 'list'));
		}



		Tpl::output('storeArray', $storeArray);
		Tpl::setDirquna('shop');
		Tpl::showpage('ownshop.add_sb');
	}
